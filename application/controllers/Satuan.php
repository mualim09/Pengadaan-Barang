...
class Satuan extends CI_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $insert = $this->admin->insert('pb_satuan', $input);
            if ($insert) {
    ...
    public function edit($getId)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Satuan";
            $data['satuan'] = $this->admin->get('pb_satuan', ['id_satuan' => $id]);
            $this->template->load('templates/dashboard', 'satuan/edit', $data);
        } else {
            $input = $this->input->post(null, true);
            $update = $this->admin->update('pb_satuan', 'id_satuan', $id, $input);
            if ($update) {
                set_pesan('data berhasil disimpan');
                redirect('satuan');
            } else {
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('pb_satuan', 'id_satuan', $id)) {
