...
class Profile extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $userId = $this->session->userdata('login_session')['pb_user'];
        $this->user = $this->admin->get('pb_user', ['id_user' => $userId]);
    }
    ...
    public function setting()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($_FILES['foto']['name'])) {
                $insert = $this->admin->update('pb_user', 'id_user', $input['id_user'], $input);
                if ($insert) {
                ...
                if ($this->upload->do_upload('foto') == false) {
                    ...
                    $input['foto'] = $gbr['file_name'];
                    $update = $this->admin->update('pb_user', 'id_user', $input['id_user'], $input);
                    if ($update) {
                        set_pesan('perubahan berhasil disimpan.');
                    } else {
    ...
    public function ubahpassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (password_verify($input['password_lama'], userdata('password'))) {
                ...
                $query = $this->admin->update('pb_user', 'id_user', userdata('id_user'), $new_pass);
