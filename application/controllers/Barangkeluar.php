...
class Barangkeluar extends CI_Controller
{
    ...
    private function _validasi()
    {
        ...
        $stok = $this->admin->get('pb_barang', ['id_barang' => $input])['stok'];
        $stok_valid = $stok + 1;
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Barang Keluar";
            $data['barang'] = $this->admin->get('pb_barang', null, ['stok >' => 0]);

            // Mendapatkan dan men-generate kode transaksi barang keluar
            $kode = 'BK-' . date('ymd');
            $kode_terakhir = $this->admin->getMax('pb_barang_keluar', 'id_barang_keluar', $kode);
            ...
            $insert = $this->admin->insert('pb_barang_keluar', $input);
    ...
    public function delete($getId)
    {
        ...
        if ($this->admin->delete('pb_barang_keluar', 'id_barang_keluar', $id)) {
