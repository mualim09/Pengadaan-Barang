...
class Supplier extends CI_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $save = $this->admin->insert('pb_supplier', $input);
            if ($save) {
    ...
    public function edit($getId)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Supplier";
            $data['supplier'] = $this->admin->get('pb_supplier', ['id_supplier' => $id]);
            $this->template->load('templates/dashboard', 'supplier/edit', $data);
        } else {
            $input = $this->input->post(null, true);
            $update = $this->admin->update('pb_supplier', 'id_supplier', $id, $input);
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('pb_supplier', 'id_supplier', $id)) {
